...
from .models import Users, Referals, Tests, Answers, Groups, Subs, Greetings, DefaultTests, DefaultAnswers
...
async def get_top_tests() -> list[Tests]:
    tests = await Tests.filter().order_by('-pass_counter')
    return tests
...
async def get_def_test(
        user_id,
        test_type   
    ):
    return await DefaultTests.get_or_none(owner=user_id, test_type=test_type)
...
async def get_random_deftest(
        user_id,
    ):
    tests_of_user = await DefaultTests.filter(owner=user_id).values('test_type')
    return tests_of_user